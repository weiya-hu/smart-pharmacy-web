  ...
  <div class="app-container">
    ...
    <div class="btn-back">
      <el-row :gutter="10" class="mb8">
        <el-col :span="1.5">
          <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['wecom:order:add']">新增
          </el-button>
        </el-col>
      ...
      <el-table-column label="操作" class-name="small-padding fixed-width" :width="270">
        <template #default="scope">
          <div class="btn-col">
            ...
            <el-button v-if="scope.row.canDelete" type="text" size="small" icon="Delete"
                       @click="handleDelete(scope.row)"
                       v-hasPermi="['wecom:order:remove']">删除
            </el-button>
            <el-button v-if="scope.row.canStop" type="text" size="small" icon="Warning"
                       @click="handleTaskStop(scope.row)"
                       v-hasPermi="['wecom:order:remove']">停用
            </el-button>
            <el-button v-if="scope.row.canStart" type="text" size="small" icon="CircleCheck"
                       @click="handleTaskBegin(scope.row)"
...
function handleQueryInfo(row) {
  router.push({
    path: '/markteCenter/activityInfo',
    query: {handleType: 'query', eventId: row.eventId, stateName: row.stateName}
  })
}
